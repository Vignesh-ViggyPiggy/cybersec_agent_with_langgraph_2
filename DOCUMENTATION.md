...
## 7. Cookbook — adding a new attack type by evidence shape

The architecture only ever supports **one primary read** (`status_file` +
`status_tag`/`status_tags`, read by `read_live_status_node`) and **at most
one verification path** per attack type (`readable_report`/
`diffable_snapshot_files` via `raw_evidence_files`, *or* `check_raw_logs`
via a log pattern — never both as independent checks; §5.1 step 4 routes
to exactly one). If your real attack has more candidate evidence sources
than that, pick the strongest one as the actual verification path; any
others become **corroborating-only** display via `raw_evidence_files` —
shown in the report whenever the graph actually reaches
`not_detected_clean_node` or `attack_info_node` (i.e. `detected`, or
`not_detected` verified clean through `readable_report`/`check_raw_logs`),
never for `opaque_binary_only`/`requires_live_recompute` types, which skip
straight to `unverifiable_path_node` and never read `raw_evidence_files`
at all on a clean result.

Only one thing here ever needs an actual code change: `check_raw_logs`
verification. Its glob patterns and matcher are hardcoded (log files
...
Everything else below is pure `corpus_documents/attack_<type>.json` data —
no code change, just `python ingest_corpus.py` afterward.
...
| 4 | No Alert.xml; XML status + log pattern | `athinio/system/example_4_output.xml` | `binary_flag` (or `count_greater_than_zero`) | `check_raw_logs` (verified against `osstatus.log`) | *(none)* | Yes — log checker |
...
| 10 | Only XML status | `athinio/system/example_10_output.xml` | `binary_flag` (or `count_greater_than_zero`) | `opaque_binary_only` | *(none)* | No |
...
Two things worth noticing across the table:

- **"A status" vs. "a particular output pattern" only changes anything
  when that file is the *primary* read** (rows 4/5, 10/11: a tag needs
  `binary_flag`/`count_greater_than_zero`, a free-text scan needs
  `file_contains_pattern` + `detected_regex`). As secondary,
  corroborating-only evidence (rows 1/2, 7/8), the distinction doesn't
  matter — `raw_evidence_files` just reads the file's raw text and hands
  it to the LLM (`readable_report`) or shows it verbatim in the report
  (`check_raw_logs` cases), regardless of whether that text is a
  structured tag or a free-text pattern.
- **Rows 1-2 vs. 7-8 look similar but differ in what actually gets
  verified.** In 1/2, `example_N_output.xml` is *never independently
  checked* — only the log pattern is; the XML file is shown for context
  only. In 7/8, `example_N_output.xml` *is* what gets checked (by an LLM,
  via `readable_report`) — there's no log at all in that shape. Don't
  read "raw_evidence_files present" as "this file is being verified" —
  check `verification_category` for that.
...
before being fixed — **confirm on a real hierarchy first**
(`ls athinio/... ` / vault-side, whichever holds `osstatus.log`, and look
for a `osstatus.log_NNN`-style suffix). If it does rotate, use
`check_raw_logs` instead: point `status_file` at any file on that
hierarchy you know reliably exists but never itself indicates detection,
so `read_live_status_node` always reports "not triggered" and the graph
falls through to `verify_raw_logs_node`'s glob-based, rotation-safe read.

### 7.2 Worked examples

**Row 1** — the richest shape (3 candidate sources, one wins as real
verification, one becomes display-only context):

```json
{
  "id": "attack_example_1",
  "explanation": "One-sentence plain description of what this attack means when detected.",
  "metadata": {
...
Plus the code-change recipe from the intro above, with `<attack_type>` =
`example_1` and your real log pattern.

**Row 5** — no Alert.xml tag at all; the primary read itself is a
text-pattern scan, and it's *also* independently verified against a log
when that scan comes back clean:
...
**Row 9** — the simplest shape, one tag, nothing else:
...
For any row, after writing the JSON (and, for rows 1-6/12, the
`check_raw_logs` code addition), run `python ingest_corpus.py` — the new
attack type is live from the next run onward, checked through the same
generic graph as every other one, with no further wiring needed.
